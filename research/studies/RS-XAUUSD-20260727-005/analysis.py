...
Usage:
    /opt/homebrew/bin/python3.12 scripts/research/build_s1_fail_pattern_gap.py
"""
...
STUDY_ID = "RS-XAUUSD-20260727-005"
V34_STUDY = "RS-XAUUSD-20260727-003"
V39_STUDY = "RS-XAUUSD-20260727-004"
...
def diff_stats_table(v34: dict, v39: dict, keys: list[str]) -> dict:
    out = {}
    for key in keys:
...
def grouped_bar_chart(labels: list[str], v34_vals: list[float], v39_vals: list[float], title: str, ylabel: str) -> plt.Figure:
    x = np.arange(len(labels))
    width = 0.35
    fig, ax = plt.subplots(figsize=(max(7, len(labels) * 1.1), 4))
    ax.bar(x - width / 2, v34_vals, width, label="V3.4", color="#95a5a6")
    ax.bar(x + width / 2, v39_vals, width, label="V3.9", color="#3498db")
...
def entry_slot_diff_chart(comparison: dict) -> plt.Figure:
    ...
    diffs = [comparison[s]["win_rate_pct_diff_v39_minus_v34"] or 0 for s in slots]
    ...
    ax.set_title("Win-Rate Diff by 30-Minute Slot (V3.9 − V3.4)")
...
def build(output_dir: Path) -> dict:
    v34_path = Path("research/studies") / V34_STUDY / "results.json"
    v39_path = Path("research/studies") / V39_STUDY / "results.json"
    ...
    entry_30m_diff = diff_stats_table(v34["by_entry_30m"], v39["by_entry_30m"], tk.ENTRY_SLOTS_30M)
    ...
    for ft in ["immediate_loss", "false_breakout", "time_bleed", "normal_sl"]:
        ...
        fail_type_share[ft] = {"v34_pct": a["pct"], "v39_pct": b["pct"], "diff": round(b["pct"] - a["pct"], 1)}

    bb_zone_diff = diff_stats_table(v34["bb_zone"], v39["bb_zone"], tk.BB_ZONE_ORDER)
    dxy_bucket_keys = sorted(set(v34["dxy"]["regime"]["by_bucket"]) | set(v39["dxy"]["regime"]["by_bucket"]))
    dxy_bucket_diff = diff_stats_table(v34["dxy"]["regime"]["by_bucket"], v39["dxy"]["regime"]["by_bucket"], dxy_bucket_keys)
    mtf_4h_diff = diff_stats_table(v34["mtf"]["by_4h_state"], v39["mtf"]["by_4h_state"], ["bearish", "neutral", "bullish"])
    mtf_conflict_diff = diff_stats_table(
        v34["mtf"]["by_conflict"], v39["mtf"]["by_conflict"],
    ...
    save(entry_slot_diff_chart(entry_30m_diff), "entry_slot_30m_diff", "30-Minute Slot Win-Rate Diff (V3.9-V3.4)")

    ft_labels = list(fail_type_share.keys())
    save(
        grouped_bar_chart(ft_labels, [fail_type_share[k]["v34_pct"] for k in ft_labels],
                           [fail_type_share[k]["v39_pct"] for k in ft_labels],
                           "Fail-Type Share: V3.4 vs V3.9", "% of losses"),
        "fail_type_share_comparison", "Fail-Type Share Comparison",
    )

    bb_labels = [z for z in tk.BB_ZONE_ORDER if bb_zone_diff[z]["v34_n"] or bb_zone_diff[z]["v39_n"]]
    save(
        grouped_bar_chart(bb_labels, [bb_zone_diff[z]["v34_win_rate_pct"] or 0 for z in bb_labels],
                           [bb_zone_diff[z]["v39_win_rate_pct"] or 0 for z in bb_labels],
                           "Win Rate by BB Zone: V3.4 vs V3.9", "Win Rate %"),
        "bb_zone_comparison", "BB Zone Win-Rate Comparison",
    )

    dxy_labels = [k for k in dxy_bucket_keys if dxy_bucket_diff[k]["v34_n"] or dxy_bucket_diff[k]["v39_n"]]
    save(
        grouped_bar_chart(dxy_labels, [dxy_bucket_diff[k]["v34_win_rate_pct"] or 0 for k in dxy_labels],
                           [dxy_bucket_diff[k]["v39_win_rate_pct"] or 0 for k in dxy_labels],
                           "Win Rate by DXY RSI Bucket: V3.4 vs V3.9", "Win Rate %"),
        "dxy_bucket_comparison", "DXY Bucket Win-Rate Comparison",
    )

    mtf_labels = [k for k in ["bearish", "neutral", "bullish"] if mtf_4h_diff[k]["v34_n"] or mtf_4h_diff[k]["v39_n"]]
    save(
        grouped_bar_chart(mtf_labels, [mtf_4h_diff[k]["v34_win_rate_pct"] or 0 for k in mtf_labels],
                           [mtf_4h_diff[k]["v39_win_rate_pct"] or 0 for k in mtf_labels],
                           "Win Rate by 4H RSI State: V3.4 vs V3.9", "Win Rate %"),
    ...
    results = {
        ...
        "strategy": "S1 AweWithBB V3.4 vs V3.9 (gap report)",
        "version_labels": {"v34": "V3.4", "v39": "V3.9"},
        "method": {
            "basis": f"reads {V34_STUDY} and {V39_STUDY} results.json; recomputes no fail-pattern classification",
            "risk_parameter_caveat": "V3.4 used 0.5% SL with TP2=2R; V3.9's exit structure differs. Deltas may reflect exit-rule changes, not only timing/regime differences.",
        },
        ...
        "sources": [
            {"role": f"solo_study_{V34_STUDY}", "path": str(v34_path), "sha256": sha256(v34_path)},
            {"role": f"solo_study_{V39_STUDY}", "path": str(v39_path), "sha256": sha256(v39_path)},
        ],
    }
...
def render_html(results: dict, charts_dir: Path) -> str:
    ...
    kpis = [
        ("V3.4 WR/PF", f'{bd["v34"]["win_rate_pct"]}% / {bd["v34"]["profit_factor"]}', "neu"),
        ("V3.9 WR/PF", f'{bd["v39"]["win_rate_pct"]}% / {bd["v39"]["profit_factor"]}', "neu"),
        ("WR diff (V3.9-V3.4)", f'{bd["win_rate_pct_diff"]:+.2f}pp', "neu"),
        ("PF diff (V3.9-V3.4)", f'{bd["profit_factor_diff"]:+.3f}', "neu"),
    ]
    ...
    for slot in tk.ENTRY_SLOTS_30M:
        ...
        if not (item["v34_n"] or item["v39_n"]):
            continue
        diff = item["win_rate_pct_diff_v39_minus_v34"]
        entry_rows.append([slot, item["v34_n"], f'{item["v34_win_rate_pct"]}%', item["v39_n"],
                            f'{item["v39_win_rate_pct"]}%', f'{diff:+.2f}pp' if diff is not None else "-"])

    fail_rows = [[k, f'{v["v34_pct"]}%', f'{v["v39_pct"]}%', f'{v["diff"]:+.1f}pp'] for k, v in results["fail_type_share_diff"].items()]

    generated_at = results["generated_at"]
    html = f"""<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>S1 AweWithBB V3.4 vs V3.9 — Gap Report</title>
{CSS}
</head>
<body>
<div class="wrap">
  <h1>S1 AweWithBB — V3.4 vs V3.9 Gap Report</h1>
  <div class="meta">Generated {generated_at} &nbsp;|&nbsp; reads {V34_STUDY} + {V39_STUDY}, recomputes nothing</div>
  {kpi_html}
  <div class="note">V3.4 used 0.5% SL with TP2=2R; V3.9's exit structure differs. Deltas may reflect exit-rule changes, not only timing/regime differences.</div>

  <h2>30-Minute Entry-Slot Win-Rate Diff</h2>
  <div class="card">
    {_img(b64["entry_slot_30m_diff"])}
    {_table(["slot", "V3.4 n", "V3.4 WR", "V3.9 n", "V3.9 WR", "diff"], entry_rows)}
  </div>

  <h2>Fail-Type Share</h2>
  <div class="card">
    {_img(b64["fail_type_share_comparison"])}
    {_table(["fail_type", "V3.4", "V3.9", "diff"], fail_rows)}
  </div>
...
def render_readme(results: dict) -> str:
    bd = results["baseline_diff"]
    lines = [
        f"# {results['study_id']} — S1 AweWithBB V3.4 vs V3.9 gap report",
        "",
        f"Generated: `{results['generated_at']}`",
        "",
        "## Scope",
        "",
        f"- Reads `{V34_STUDY}` and `{V39_STUDY}` results.json; recomputes no fail-pattern classification.",
        f"- Baseline: V3.4 WR {bd['v34']['win_rate_pct']}% / PF {bd['v34']['profit_factor']}; "
        f"V3.9 WR {bd['v39']['win_rate_pct']}% / PF {bd['v39']['profit_factor']} "
        f"(diff {bd['win_rate_pct_diff']:+.2f}pp / {bd['profit_factor_diff']:+.3f}).",
        "- **Risk-parameter caveat**: V3.4 used 0.5% SL with TP2=2R; V3.9's exit structure "
        "differs. Deltas may reflect exit-rule changes, not only timing/regime differences.",
        "",
        "## Interpretation",
        "",
        "- See `report.html` for the full chart-embedded comparison and `results.json` for structured deltas.",
        "- Only V3.9 is eligible to affect the live S1 advisory score.",
        "",
    ]
    return "\n".join(lines) + "\n"
