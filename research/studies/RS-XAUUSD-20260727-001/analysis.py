...
field for baseline, fail-pattern breakdown, 30-minute timing, BB zone, DXY regime, MTF
alignment, the Macro composite verdict, and the integer rank_score carried by
by_entry_30m/by_session/by_macro_verdict. Chart PNGs are published as pre-verified
static files and are not regenerated here; the executor manually reviewed the
chart-generation code path (no file paths in any chart title/axis) before publishing
them — see the Private decision_log.md.
"""
