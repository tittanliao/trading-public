...
# ── 路徑 ──────────────────────────────────────────────
WEEKLY_CSV = Path("csv/TAIFEX_DLY_MXF1!, 1W.csv")
...
def load_weekly():
    ...
    num_cols = ['open', 'high', 'low', 'close', 'RSI', 'RSI-based MA', 'Upper', 'Lower', 'Basis']
    for c in num_cols:
        if c in df.columns:
    ...
    df['week_of_month'] = df.groupby(['year', 'month']).cumcount() + 1  # 1–5
...
def build_monthly(df):
    ...
    monthly['chg_pts']  = monthly['week_close'] - monthly['week_open']
    monthly['chg_pct']  = monthly['chg_pts'] / monthly['week_open'] * 100
...
def seasonality(monthly):
    ...
    for m in range(1, 13):
        ...
        avg_chg   = sub['chg_pts'].mean()
        avg_pct   = sub['chg_pct'].mean()
        med_chg   = sub['chg_pts'].median()
        best      = sub['chg_pts'].max()
        worst     = sub['chg_pts'].min()
        n         = len(sub)
        bias      = 'LONG' if win_rate >= 55 else ('SHORT' if win_rate <= 45 else 'NEUTRAL')
        rows.append(dict(
            month=m, month_name=month_names[m-1],
            n=n, win_rate=win_rate,
            avg_chg=avg_chg, avg_pct=avg_pct,
            med_chg=med_chg, best=best, worst=worst,
            bias=bias
        ))
...
# ── 週內結構（每月第 1–5 週的強弱）────────────────────
def week_in_month_stats(df):
    df2 = df.copy()
    df2['week_chg'] = df2['close'] - df2['open']
    df2['week_bull'] = df2['week_chg'] > 0
    grp = df2.groupby('week_of_month').agg(
        n         = ('week_chg', 'count'),
        win_rate  = ('week_bull', 'mean'),
        avg_chg   = ('week_chg', 'mean'),
        med_chg   = ('week_chg', 'median'),
    ).reset_index()
    grp['win_rate'] = grp['win_rate'] * 100
    grp = grp[grp['week_of_month'] <= 5]
    return grp


# ── 近 12 個月回顧 ───────────────────────────────────
def recent_12m(monthly):
...
def pct_bar(val, max_val=500, positive_color='#27ae60', negative_color='#e74c3c'):
    width = min(abs(val) / max_val * 100, 100)
    color = positive_color if val >= 0 else negative_color
    return f'<div style="display:inline-block;width:{width:.1f}px;height:10px;background:{color};vertical-align:middle;"></div>'


def render_seasonality_table(sea):
    rows = ''
    for _, r in sea.iterrows():
        ...
        <tr>
          ...
          <td>{r['avg_chg']:+.0f} pts {pct_bar(r['avg_chg'])}</td>
          <td>{r['avg_pct']:+.2f}%</td>
          <td>{r['med_chg']:+.0f}</td>
          <td style="color:#27ae60">{r['best']:+.0f}</td>
          <td style="color:#e74c3c">{r['worst']:+.0f}</td>
          <td><span style="background:{bias_color};color:#fff;padding:2px 8px;border-radius:4px;font-weight:bold">{bias_label}</span></td>
        </tr>"""
    return f"""
    <table class="data-table">
      <thead><tr>
        <th>月份</th><th>樣本數</th><th>月勝率</th><th>平均漲跌（點）</th>
...
def render_week_in_month_table(wim):
    ...
    for _, r in wim.iterrows():
        ...
        <tr>
          ...
          <td>{r['avg_chg']:+.0f} pts {pct_bar(r['avg_chg'], max_val=200)}</td>
          <td>{r['med_chg']:+.0f}</td>
        </tr>"""
    return f"""
    <table class="data-table">
      <thead><tr>
        <th>週次</th><th>樣本數</th><th>週勝率</th><th>平均漲跌（點）</th><th>中位數</th>
      </tr></thead>
      <tbody>{rows}</tbody>
    </table>"""


def render_recent_table(rec):
    rows = ''
    for _, r in rec.iterrows():
        ...
        <tr>
          ...
          <td style="color:{color};font-weight:bold">{sign} {r['chg_pts']:+.0f} pts ({r['chg_pct']:+.1f}%)</td>
...
def render_monthly_heatmap(monthly):
    ...
    def chg_to_color(v):
        ...
        if v >  500: return '#0a5c2e'
        if v >  200: return '#1a7a4a'
        if v >    0: return '#27ae60'
        if v > -200: return '#e74c3c'
        if v > -500: return '#c0392b'
        return '#7b241c'
    ...
    for y in years:
        ...
        for m in range(1, 13):
            ...
            if len(sub) == 0:
                ...
                v = sub['chg_pts'].values[0]
                pct = sub['chg_pct'].values[0]
                bg = chg_to_color(v)
                txt = f'{v:+.0f}'
                row += f'<td style="background:{bg};color:#fff;font-size:11px" title="{pct:+.1f}%">{txt}</td>'
        row += '</tr>'
        body += row
    ...
    <p style="font-size:12px;color:#888">顏色：深綠 &gt;500pts ／ 綠 &gt;200 ／ 淺綠 &gt;0 ／ 紅 &lt;0 ／ 深紅 &lt;-500。滑鼠懸停顯示 % 漲跌。</p>"""


def render_current_month_dashboard(monthly, sea, wim):
    """當前月份的交易偏向建議"""
    ...
    # 當前月進行到哪一週
    # 找今年本月所有週
    cur_weeks = monthly[(monthly['year']==cur_year) & (monthly['month']==cur_month)]
    ...
    <div class="dashboard-card">
      ...
      <p>月勝率：<b>{sea_row['win_rate']:.1f}%</b>　平均漲跌：<b>{sea_row['avg_chg']:+.0f} pts</b>　中位數：<b>{sea_row['med_chg']:+.0f} pts</b></p>
      <p>{rsi_state}</p>
...
def build_html(df, monthly, sea, wim):
    ...
    dashboard    = render_current_month_dashboard(monthly, sea, wim)

    html = f"""<!DOCTYPE html>
<html lang="zh-TW">
<head>
<meta charset="UTF-8">
<title>TX 宏觀分析 — 月度方向 + 週內結構</title>
<style>
...
<div class="stats-row">
  ...
  <div class="stat-card">
    <div class="val" style="color:{'#27ae60' if avg_chg>0 else '#e74c3c'}">{avg_chg:+.0f}</div>
    <div class="lbl">平均月漲跌（點數）</div>
  </div>
  <div class="stat-card">
...
<div class="section">
  ...
  <p class="note">月勝率 ≥55% → 偏多，≤45% → 偏空，中間 → 中性。樣本數 = 歷史年份數。</p>
  {season_table}
</div>
...
def main():
    ...
    # 印出季節性摘要
    print("\n=== 季節性偏向摘要 ===")
    print(f"{'月份':<6} {'勝率':>6} {'平均點':>8} {'偏向':>6}")
    print("─" * 30)
    for _, r in sea.iterrows():
        print(f"{r['month_name']:<6} {r['win_rate']:>5.1f}% {r['avg_chg']:>+8.0f} pts  {r['bias']}")

    print("\n=== 週內結構 ===")
    wlabel = {1:'第1週',2:'第2週',3:'第3週',4:'第4週',5:'第5週'}
    for _, r in wim.iterrows():
        print(f"{wlabel[int(r['week_of_month'])]}: 勝率 {r['win_rate']:.1f}%  平均 {r['avg_chg']:+.0f} pts")
